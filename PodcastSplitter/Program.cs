...
namespace PodcastSplitter
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Debug.WriteLine(string.Format("mp3split_split_options: {0}", config["mp3splt_split_options"]));

            string options = string.Format("{0} -d {1} -o {2} {3}",
                config["mp3splt_split_options"],
                config["outputdir"], 
                config["mp3splt_output_options"],
                string.Format("\"{0}\"", podcasts));
            
            Process p = new Process();
            p.StartInfo.FileName = config["mp3splt"];
